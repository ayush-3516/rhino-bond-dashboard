...
serve(async (req) => {
  ...
  try {
    ...
    if (productError || !product || !product.is_active) {
      return new Response(JSON.stringify({ error: 'Invalid or inactive product' }), {
        status: 404,
        headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
      })
    }
    ...
    return new Response(JSON.stringify({ message: 'QR codes generated successfully', batchId }), {
      status: 201,
      headers: {
    ...
    return new Response(JSON.stringify({ error: 'Failed to generate QR codes' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' },
    })
  }
